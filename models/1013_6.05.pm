...
class UnetGeneratorMMU(nn.Module):
	...
	self.en1 = EnLayer(output_nc, ngf, input_nc=input_nc, norm_layer=norm_layer, outermost=True) #128
	self.t1 = TransLayer(output_nc, ngf, input_nc=input_nc, norm_layer=norm_layer, outermost=True)
	self.en2 = EnLayer(ngf, ngf * 2, input_nc=None, norm_layer=norm_layer) #64
	self.t2 = TransLayer(ngf, ngf * 2, input_nc=None, norm_layer=norm_layer)
	self.en3 = EnLayer(ngf * 2, ngf * 4, input_nc=None,norm_layer=norm_layer) #32
	self.t3 = TransLayer(ngf * 2, ngf * 4, input_nc=None,norm_layer=norm_layer)
	self.en4 = EnLayer(ngf * 4, ngf * 8, input_nc=None, norm_layer=norm_layer)#16
	self.t4 = TransLayer(ngf * 4, ngf * 8, input_nc=None, norm_layer=norm_layer)
    ...
    def forward(self, input, input2):
 	    oe1 = self.en1(input)  #128 te2 
	    te2, oe2 = self.en2(oe1) 
	    te3, oe3 = self.en3(oe2) 
	    te4, oe4 = self.en4(oe3) 
	    ...
	    od3 = torch.cat([self.de3(od4), te3],1)	  
	    od2 = torch.cat([self.de2(od3), te2],1)	  
	    od1 = self.de1(od2)
	    return(od1)
	    ...
	    od8 = torch.cat([self.de8(oe8), x8],1)	  
	    od7 = torch.cat([self.de7(od8), x7],1)	
	    od6 = torch.cat([self.de6(od7), x6],1)
	    od5 = torch.cat([self.de5(od6), x5],1)	  
	    od4 = torch.cat([self.de4(od5), x4],1)	  
	    od3 = torch.cat([self.de3(od4), x3],1)	  
	    od2 = torch.cat([self.de2(od3), x2],1)	  
	    od1 = self.de1(od2)
	    return(od1)
